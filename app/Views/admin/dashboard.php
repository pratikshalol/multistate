...
<div class="admin-section">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="admin-panel admin-panel-pad flex items-center justify-between">
            <div>
                ...
                <div class="text-[11px] text-brand-600 font-semibold mt-0.5"><?= $new_accounts ?> Pending New</div>
            </div>
            <div class="w-12 h-12 bg-brand-100 text-brand-600 rounded-xl flex items-center justify-center text-xl font-bold">📋</div>
        </div>
        ...
        <div class="admin-panel admin-panel-pad flex items-center justify-between">
            <div>
                ...
                <div class="text-[11px] text-brand-600 font-semibold mt-0.5"><?= $new_enquiries ?> Unread</div>
            </div>
            <div class="w-12 h-12 bg-brand-100 text-brand-600 rounded-xl flex items-center justify-center text-xl font-bold">📬</div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-7 admin-panel">
            <div class="p-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <a href="<?= base_url('admin/accounts') ?>" class="text-xs font-bold text-brand-600 hover:underline">View All &rarr;</a>
            </div>
                                ...
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="p-3">
                                        <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase 
                                            <?= $app['status'] === 'new' ? 'bg-brand-100 text-brand-800' : '' ?>
                                            <?= $app['status'] === 'approved' ? 'bg-brand-100 text-brand-800' : '' ?>
                                            <?= $app['status'] === 'contacted' ? 'bg-brand-100 text-brand-800' : '' ?>
                                            <?= $app['status'] === 'rejected' ? 'bg-red-100 text-red-800' : '' ?>">
                                            <?= esc($app['status']) ?>
                                        </span>
                                    </td>
                                    <td class="p-3 text-right">
                                        <a href="<?= base_url('admin/accounts/view/' . $app['id']) ?>" class="text-slate-700 hover:text-brand-600 font-bold">View</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
        ...
        <div class="lg:col-span-5 admin-panel">
            <div class="p-4 border-b border-slate-100 flex items-center justify-between">
                <h2 class="font-bold text-sm text-slate-900 flex items-center space-x-2">
                    <span>📬</span>
                    <span>Recent Contact Enquiries</span>
                </h2>
                <a href="<?= base_url('admin/enquiries') ?>" class="text-xs font-bold text-brand-600 hover:underline">View All &rarr;</a>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs">
                    <thead class="admin-table-head">
                        ...
                        <?php if (!empty($recent_enquiries)): ?>
                            <?php foreach ($recent_enquiries as $enq): ?>
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="p-3 text-right">
                                        <a href="<?= base_url('admin/enquiries/view/' . $enq['id']) ?>" class="text-slate-700 hover:text-brand-600 font-bold">Details</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
