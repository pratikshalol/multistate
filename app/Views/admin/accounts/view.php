...
<div class="admin-section">
    <div class="admin-head">
        <div>
            <h2 class="admin-head-title">Account Application Details</h2>
        ...
        <div class="p-6 space-y-6">
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="rounded-2xl bg-slate-50 p-4">
                    ...
                    <span class="mt-2 inline-flex rounded-full px-3 py-1 text-[11px] font-bold <?= $application['status'] === 'new' ? 'bg-brand-100 text-brand-800' : ($application['status'] === 'contacted' ? 'bg-brand-100 text-brand-800' : ($application['status'] === 'approved' ? 'bg-brand-100 text-brand-800' : 'bg-red-100 text-red-800')) ?>">
                ...
                <div class="rounded-2xl bg-slate-50 p-4">
                    ...
                    <p class="mt-2 font-semibold text-brand-700"><?= esc($application['mobile']) ?></p>
                </div>
                <div class="rounded-2xl bg-slate-50 p-4">
            ...
            <form action="<?= base_url('admin/accounts/update-status/' . $application['id']) ?>" method="post" class="grid gap-4 md:grid-cols-2">
                <div class="rounded-2xl bg-slate-50 p-6">
                    ...
                    <select name="status" class="mt-3 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 focus:border-brand-300 focus:ring-brand-200">
                ...
                <div class="flex items-end justify-end">
                    <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-brand-600 px-5 py-3 text-sm font-bold text-white hover:bg-brand-600 transition">
                        Save Status
                    </button>
                </div>
            </form>
