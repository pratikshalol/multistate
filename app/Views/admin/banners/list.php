...
<div class="admin-section">
    <div class="admin-head">
        <h2 class="admin-head-title">Hero Banner Slides</h2>
        <a href="<?= base_url('admin/banners/create') ?>" class="btn-admin shadow">
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="p-4 text-slate-600">
                                <?php if (!empty($banner['cta_text'])): ?>
                                    <span class="font-semibold text-brand-700"><?= esc($banner['cta_text']) ?></span>
                            ...
                            <td class="p-4">
                                <span class="px-2 py-0.5 rounded text-[10px] font-bold <?= $banner['is_active'] ? 'bg-brand-100 text-brand-800' : 'bg-slate-100 text-slate-500' ?>">
                                    <?= $banner['is_active'] ? 'ACTIVE' : 'INACTIVE' ?>
                                </span>
                            </td>
                            <td class="p-4 text-right space-x-2 whitespace-nowrap">
                                <a href="<?= base_url('admin/banners/edit/' . $banner['id']) ?>" class="btn-row">Edit</a>
                                <button onclick="confirmDelete('<?= base_url('admin/banners/delete/' . $banner['id']) ?>')" class="btn-row-danger">Delete</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="p-10 text-center text-slate-400 font-semibold">
                            No banner slides yet. <a href="<?= base_url('admin/banners/create') ?>" class="text-brand-600 hover:underline font-bold">Add your first one →</a>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
